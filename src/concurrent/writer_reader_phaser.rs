...
impl WriterReaderPhaser {
    ...
    pub fn begin_writer_critical_section<'a>(&'a self) -> WriterCriticalSectionGuard<'a> {
        let critical_value = self.start_epoch.fetch_add(1, Ordering::Release);
        if critical_value < 1 {
            WriterCriticalSectionGuard { epoch: &self.odd_end_epoch }
        }
        else {
            WriterCriticalSectionGuard { epoch: &self.even_end_epoch }
        }
    }
...
impl<'a> PhaseFlipGuard<'a> {
    pub fn flip_with_yield_time(&self, yield_time: Duration) {
        let next_phase_is_even = self.parent.start_epoch.load(Ordering::Relaxed) < 0;
        ...
        let start_value_at_flip = self.parent.start_epoch.swap(initial_start_value, Ordering::AcqRel);
